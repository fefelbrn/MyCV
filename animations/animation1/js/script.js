// Keep the frame borders fixed even when the user zooms
// This was a pain to get right, browsers handle zoom weirdly
(function() {
    'use strict';
    ...
    const OFFSET = 15; // Distance from edges
    
    let isZooming = false;
    let zoomTimeout = null;
    
    // Try different methods to detect zoom, browsers are inconsistent
    function getZoomLevel() {
        const zoom1 = window.outerWidth / window.innerWidth;
        const zoom2 = window.devicePixelRatio || 1;
        
        // This one seems most reliable - create a test element and measure it
    ...
    function updateFramePositions() {
        ...
        // Apply inverse scale so the frame stays the same visual size when zoomed
        frameLayer.style.transform = `scale(${inverseZoom})`;
        frameLayer.style.transformOrigin = 'top left';
        
        // Adjust size to account for the scale transform
    ...
    // Keep checking for zoom changes, browsers are sneaky
    ...
    // Use Visual Viewport API if available (mobile mostly)
